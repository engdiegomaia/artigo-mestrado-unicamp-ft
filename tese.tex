...
\titulo{Análise Comparativa de Processamento Hiperespectral: Implementação em FPGA versus GPU}
...
\begin{abstract}
This work presents a comparative analysis between FPGA and GPU implementations for hyperspectral image processing. Hyperspectral image processing demands high computational capacity due to data volume and algorithm complexity. This dissertation implements and compares preprocessing, dimensionality reduction (PCA), and classification (SVM) algorithms on both platforms, using GHDL for FPGA simulation and CUDA for GPU implementation. Experimental results demonstrate trade-offs between performance, energy consumption, and accuracy, providing guidelines for choosing the most suitable platform according to application scenarios.

\textbf{Keywords:} Hyperspectral Processing, FPGA, GPU, CUDA, VHDL, Comparative Analysis.
\end{abstract}
